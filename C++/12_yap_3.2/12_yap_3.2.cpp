...
void MyQueue::Info() // Информация о объеме и цене первой партии и кол-ве партий
{
    ...
    else
    {
        ...
        cout << "\tКоличество единиц = " << First->data.volume << endl;
        cout << "\tСтоимость единицы товара = " << First->data.price << endl << endl;
    }
}
...
int main()
{
    ...
    while (m)
    {
        ...
        {
            system("cls");
            cout << "Введите кол-во товаров, а затем стоимость:" << endl;
            cout << "Кол-во = "; cin >> dt.volume;
            ...
            if (check(Q, dt_x))
            {
                while (dt_x.volume)
                {
                    if (dt_x.volume <= dt.volume)
                    {
                        ...
                        Q.full_cost -= dt.price * dt_x.volume;
                    ...
                    else
                    {
                        ...
                        Q.full_cost -= dt.price * dt.volume;
                        dt_x.volume -= dt.volume;
                        dt.volume = 0;
                    }
                    if (dt.volume == 0)
        ...
        {
            system("cls");
            cout << "Проданные товары обошлись в = " << Q.full_cost << endl;
